...
set_version() {
    ...
    if [ "$RELEASE_TYPE" == "Full" ]; then
       echo "RELEASE_TYPE = '$RELEASE_TYPE'"
       perl -pi -e "s/com.github.kaltura:kaltura-player-android:.*?-SNAPSHOT/com.kaltura.player:tvplayer:$NEW_VERSION/" $BUILD_GRADLE
    fi
}
...
EOF
    ...
    if [ "$RELEASE_TYPE" = "Update" ]; then
                      JSON_BODY="### Smartswitch Plugin Support\n\n"
                      JSON_BODY="$JSON_BODY$NEW_TAG\n\n"
          JSON_BODY="$JSON_BODY * upgrade to $NEW_TAG\n\n"
          JSON_BODY="$JSON_BODY #### Gradle\n\n"
                      JSON_BODY="$JSON_BODY * implementation 'com.kaltura.playkit:smartswitchplugin"
      ...
      "sections": [
          {
              "facts": [
                  ...
                  {
                      "name": "Gradle line",
                      "value": "implementation 'com.kaltura.playkit:smartswitchplugin:$COMMIT_SHA'"
                  }
              ]
          }
      ],
